...
# Updated CSS with a new white and #02899d color scheme
st.markdown("""
<style>
/* Define the new color palette */
:root {
  --primary-color: #02899d;
  --primary-dark-color: #015c68; /* Darker shade for the main heading */
  --light-accent: #e6f4f5;      /* A light teal for backgrounds/hovers */
...
section.main > div > div > div > div{
    ...
    box-shadow: 0 4px 12px rgba(2, 137, 157, 0.1); /* Shadow tinted with primary color */
...
/* Main title color - using the new darker shade */
h1 { color: var(--primary-dark-color) !important; }

/* Sub-headings use the primary color */
...
/* Style for bold text, used for table headers and some labels */
strong {
...
/* active button state - for toggled mode buttons */
...
def show_clickable_table(df_:pd.DataFrame, header:str|None=None, key_prefix="tbl"):
    ...
    # The new CSS styles the <strong> tag, making the headers colored
...
with st.container():
    ...
    const t=setInterval(()=>{{
        ...
        const activeBtn = [...buttons].find(e => e.innerText.trim() === "{active_lbl}");
        if(activeBtn){{
